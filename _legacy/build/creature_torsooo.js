...
function thingStand(thing) {
    thing.mainpart.body.static = true;
}
function thingRest(thing) {
    thing.mainpart.body.static = false;
}
function thingRotateLeft(thing) {
    thing.mainpart.body.static = true;
    thing.mainpart.body.rotateLeft(5000);
    thing.mainpart.body.static = false;
}
function thingRotateRight(thing) {
    thing.mainpart.body.static = true;
    thing.mainpart.body.rotateRight(5000);
    thing.mainpart.body.static = false;
}
...
function agentMove(act) {
    if (act == 1) {
        torso.mainpart.body.moveForward(500);
    }
    else if (act == 2) {
        torso.mainpart.body.moveBackward(500);
    }
    else if (act == 3) {
        torso.mainpart.body.rotateLeft(500);
    }
    else if (act == 4) {
        torso.mainpart.body.rotateRight(500);
    }
    else {
        stay='none';
    }
...
function wwwMove(act) {
    if (0 <= act <= 0.2) {
        www.body.moveForward(wwwmoveV);
    }
    else if (0.2 < act <= 0.4) {
        www.body.moveForward(wwwmoveV);
    }
    else if (0.4 < act <= 0.6) {
        www.body.rotateLeft(wwwmoveV);
    }
    else if (0.6 < act <= 0.8) {
        www.body.rotateRight(wwwmoveV);
    }
    else {
    }
}
...
function update() {
    ...
    s3=Phaser.Math.distance(torso.mainpart.body.x, torso.mainpart.body.y, mouse.body.x, mouse.body.y);
    s4=Phaser.Math.angleBetween(torso.mainpart.body.x, torso.mainpart.body.y, mouse.body.x, mouse.body.y);
    s5=Phaser.Math.distance(www.body.x, www.body.y, mouse.body.x, mouse.body.y);
    s6=Phaser.Math.angleBetween(www.body.x, www.body.y, mouse.body.x, mouse.body.y);
...
function render() {
    ...
    game.debug.text(s1+"  "+s2+"  "+s3+"  "+s4+"  "+s5+"  "+s6, 32, 64);
